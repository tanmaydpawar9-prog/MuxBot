...
async def help_cmd(client, message):
    await message.reply("❓ **Usage:**\n1. Use `/mux`\n2. Send Video\n3. Send Sub\n4. Type Name (ex: `Shroud EP157 [4K]`)\n5. Send Thumb or /skip")

@app.on_message(filters.command("access") & filters.private)
async def access_cmd(client, message):
    status = "✅ AUTHORIZED" if message.from_user.id in AUTHORIZED_USERS else "❌ UNAUTHORIZED"
...
# --- MESSAGE HANDLER ---
@app.on_message(filters.private & ~filters.command(["style", "convert", "mux", "help", "access", "about", "guide"]) & filters.user(AUTHORIZED_USERS))
...
async def finalize_mux(client, message, chat_id):
    ...
    try:
        ...
        s_msg = await client.get_messages(chat_id, data["sub_id"])
        v_p = await client.download_media(v_msg, file_name=f"{work_dir}/v.mp4", progress=progress_bar, progress_args=(status, time.time(), "Downloading"))
        ...
        await client.send_document(chat_id, out_path, thumb=th, caption=cap, progress=progress_bar, progress_args=(status, time.time(), "Uploading"))
