...
# --- STYLES DEFINITION ---
# Format: Fontname, Fontsize, PrimaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
STYLE_CINEMATIC = "Arial,20,&H0000FFFF,&H00000000,&H00000000,1,0,0,0,100,100,0,0,1,2,2,2,10,10,10,1"
STYLE_REGULAR = "Arial,18,&H00FFFFFF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,1,1,2,10,10,10,1"
...
async def collector(client, message):
    ...
    if message.video or (message.document and "video" in (message.document.mime_type or "")):
        ...
        if chat_id in user_data and user_data[chat_id]["step"] == "sub":
            ...
            buttons = []
            if ext == "vtt":
                buttons = [[InlineKeyboardButton("Convert to SRT", callback_data="conv_srt"), InlineKeyboardButton("Convert to ASS", callback_data="conv_ass")]]
            elif ext == "srt":
                buttons = [[InlineKeyboardButton("Convert to VTT", callback_data="conv_vtt"), InlineKeyboardButton("Convert to ASS", callback_data="conv_ass")]]
            else: # Already .ass
...
async def handle_choices(client, callback_query):
    ...
    if data.startswith("conv_"):
        ...
        if fmt == "ass":
            buttons = [[InlineKeyboardButton("🎬 Cinematic", callback_data="style_cinematic"), InlineKeyboardButton("📝 Regular", callback_data="style_regular")]]
            await callback_query.message.edit("Select **ASS Style**:", reply_markup=InlineKeyboardMarkup(buttons))
        else:
            user_data[chat_id]["step"] = "name"
        ...
        user_data[chat_id]["step"] = "name"
        await callback_query.message.edit("Style selected. Type the **Final File Name**:", reply_markup=ForceReply(True))
...
async def process_final(client, message):
    ...
    await client.send_video(chat_id=chat_id, video=output, thumb=thumb_path, caption=f"**{output}**\n\n@TheFrictionRealm", progress=progress_bar, progress_args=(status, time.time(), "Uploading"), supports_streaming=True)
